...
const VideoPlayer: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const shakaInitTimeout = setTimeout(() => {
      if (src.startsWith("blob:") || src.startsWith("data:")) {
        // Native playback for locally downloaded standard MP4/MP3 files
        video.src = src;
        video.load();
        
        const savedTime = ContentId ? parseFloat(localStorage.getItem(`lectureTime_${ContentId}`) || "0") : 0;
      ...
      // Extract query parameters from src if signedUrlQuery is not provided
      let effectiveQuery = signedUrlQuery || "";
      if (!effectiveQuery && src.includes("?")) {
        try {
          const urlObj = new URL(src);
          effectiveQuery = urlObj.search;
        } catch (e) {
          const qIdx = src.indexOf("?");
          if (qIdx !== -1) {
            effectiveQuery = src.slice(qIdx);
          }
        }
      }
      const proxyUri = (uri: string) => {
        let finalUri = uri;
        if (effectiveQuery && !uri.includes(effectiveQuery)) {
          finalUri = uri.includes("?")
            ? `${uri}&${effectiveQuery.slice(1)}`
            : `${uri}${effectiveQuery}`;
        }
        
        if (finalUri.includes("cloudfront.net") || finalUri.includes("penpencil.co") || finalUri.includes("classplus")) {
          return `/api/proxy?url=${encodeURIComponent(finalUri)}`;
        }
        return finalUri;
      };
          ...
          if (!fallbackApplied && drmConfig?.clearKeys) {
            ...
            try {
              ...
              newPlayer.configure({
                ...
                streaming: {
                  bufferingGoal: 300, // Buffer 300 seconds (approx. 30-50 chunks) ahead
                  rebufferingGoal: 0.2, // Rebuffer 0.2 seconds before play resume (instant play!)
                  bufferBehind: 60, // Keep 60 seconds of buffer behind
                  segmentPrefetchLimit: 3, // Fetch up to 3 segments in parallel to avoid connection starvation
                  jumpLargeGaps: true, // Automatically jump over gaps
                }
              });

              newPlayer
                .getNetworkingEngine()
                ?.registerRequestFilter((requestType: number, request: shaka.extern.Request) => {
                  if (requestType === shaka.net.NetworkingEngine.RequestType.LICENSE) return;
                  if (request.uris) {
                    request.uris = request.uris.map(proxyUri);
                  }
                });

              const savedTime = ContentId ? parseFloat(localStorage.getItem(`lectureTime_${ContentId}`) || "0") : 0;
              const mimeType = src.includes(".m3u8") ? "application/x-mpegurl" : "application/dash+xml";
              await newPlayer.load(src, savedTime, mimeType);
      ...
      const loadVideo = async () => {
        try {
          ...
          player.getNetworkingEngine()?.registerRequestFilter((requestType: number, request: shaka.extern.Request) => {
            if (requestType === shaka.net.NetworkingEngine.RequestType.LICENSE) return;
            if (request.uris) {
              request.uris = request.uris.map(proxyUri);
            }
          ...
          const savedTime = ContentId ? parseFloat(localStorage.getItem(`lectureTime_${ContentId}`) || "0") : 0;
          const mimeType = src.includes(".m3u8") ? "application/x-mpegurl" : "application/dash+xml";
          await player.load(src, savedTime, mimeType);

          const durationToSet = !isNaN(video.duration) && video.duration > 0 ? video.duration : player.duration();
          ...
          console.error("Error loading video:", err.code, err.message, err);
        }
      };
  ...
  }, [src, type, signedUrlQuery, JSON.stringify(drmConfig)]);
  ...
  useEffect(() => {
    if (ContentId && currentTime > 0) {
      localStorage.setItem(`lectureTime_${ContentId}`, currentTime.toString());
    }
  }, [currentTime, ContentId]);
                              ...
                              <div className="bg-[#17171C]">
                                <div className="text-[#d9d9da] max-h-[200px] overflow-y-auto">
                                  <div className="flex items-center justify-between py-2 px-4 hover:bg-[#1E1E24] cursor-pointer"
                                    ...
                                    <div className="py-[10px] pl-3 pr-[6px] flex items-center gap-1 rounded-md">
                                    ...
                                    <div className="py-[10px] px-[6px] pl-3 flex items-center gap-1 rounded-md">
                                      <span className="w-[86px] text-sm font-medium capitalize leading-5 text-[#d9d9da] truncate">
                                        {selectedQuality === "auto"
                                          ? "Auto"
                                          : availableQualities.find((q) => q.id === selectedQuality)?.label ?? "Unknown"}
                                      </span>
                                    </div>
                                  </div>
                                </div>
