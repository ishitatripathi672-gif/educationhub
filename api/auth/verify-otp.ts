...
// Fire‑and‑forget batch sync after OTP verification
async function syncUserBatches(user: any, realAccessToken: string, realRefreshToken: string) {
  // Helper to fetch purchased batches
  async function fetchPurchasedBatches(accessToken: string) {
    const rid = uuidv4();
    const response = await fetch(`${BASE_URL}/batch-service/v1/batches/purchased-batches?page=1&type=ALL&amount=paid`, {
      method: "GET",
      headers: {
  ...
  await Batch.updateMany(
    { "enrolledTokens.ownerId": user._id },
    {
      $set: {
        ...
        "enrolledTokens.$[elem].randomId": uuidv4(),
...
export default async function handler(
  ...
  try {
    ...
    for (const batch of purchasedBatches) {
      ...
      // Automatically enroll the user in their purchased batch
      const alreadyEnrolled = user.enrolledBatches?.some(
        (b: any) => b.batchId === batch._id
      );
      if (!alreadyEnrolled) {
    ...
    // Fire-and-forget batch sync after OTP verification
    syncUserBatches(user, realAccessToken, realRefreshToken);
